...
const SliderImage = ({ images, custom, handleStyleChange }) => {
			...
			<ul className='flex grid-col-3 gap-6 mb-8 px-2'>
				{displayedImages.map((item, id) => (
					<li key={id} className='flex-grow'>
						<button
							className={`w-full ${selectedImage === item.img ? 'border-yellow-400 border-4' : ''}`}
							onClick={() => handleImageClick(item.img, item.style)}
						>
							<Image
								src={item.img}
								alt='img'
